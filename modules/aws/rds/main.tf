...
resource "datadog_monitor" "rds_max_connections" {
  ...
  query = "avg(last_10m):avg:aws.rds.database_connections{dbclusteridentifier:${var.aws_rds_cluster_name}} by {environment,application} >= 600"
...
resource "datadog_monitor" "rds_deadlocks" {
  ...
  query = "avg(last_10m):avg:aws.rds.deadlocks{dbclusteridentifier:${var.aws_rds_cluster_name}} by {environment,application}.as_count() >= 0.5"
